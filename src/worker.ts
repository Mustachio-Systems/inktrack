...
export interface Env {
  DB: D1Database;
  JWT_SECRET: string;       // Asserts environment token configuration
}
...
const CryptoUtils = {
  ...
  async verifyToken(token: string, secret: string): Promise<{ artistId: string } | null> {
    try {
      ...
      if (Math.floor(Date.now() / 1000) > payload.exp) return null; // Token expiration verification
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/api/auth/signup') {
        ...
        ).bind(artistId, normalizedEmail, passwordHash, artistName).run();
      ...
      if (request.method === 'POST' && url.pathname === '/api/auth/login') {
        ...
        const artist = await env.DB.prepare('SELECT id, password_hash FROM artists WHERE email = ?').bind(normalizedEmail).first<any>();
      ...
      if (request.method === 'POST' && url.pathname === '/api/auth/forgot') {
        ...
        if (artist) {
          ...
          ).bind(resetId, artist.id, resetToken, expiresAt).run();
      ...
      if (request.method === 'GET' && url.pathname === '/api/transactions') {
        const { results } = await env.DB.prepare(
          'SELECT id, timestamp, clientName, description, incomeType, paymentMethod, grossAmount, shopCutPercentage, netAmount FROM transactions WHERE artist_id = ? ORDER BY timestamp DESC'
        ).bind(artistId).all();
      ...
      if (request.method === 'POST' && url.pathname === '/api/transactions') {
        ...
        await env.DB.prepare(
          `INSERT INTO transactions (id, artist_id, timestamp, clientName, description, incomeType, paymentMethod, grossAmount, shopCutPercentage, netAmount)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
        ).bind(
          data.id,
          artistId, 
